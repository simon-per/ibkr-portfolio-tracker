...
| the dividend chart block | the Dividends tab draws the same stacked picture twice — monthly buckets and rolling twelve-month windows, same symbols, same colours, same solid/dashed forecast split. Rebuilding the rolling view as a stack would have made it two near-identical fifty-line recharts trees plus two copies of the tooltip's payload plumbing, on top of an axis formatter **already** duplicated character-for-character between the two files and a chart-height literal written out four times against the rule in `docs/frontend.md`. Caught **while writing the second copy**, which is the cheap moment. Extracted to `DividendStackChart.tsx` on 2026-09-18. Note what no test could have caught: `DividendsTab.test.tsx` mocks recharts' `ResponsiveContainer` to an empty div, so no `<Bar>` has ever mounted in the suite — a series added to one view and not the other would have rendered nothing, on a chart nobody was looking at, with a green suite |
| the dividend palette and its slot count | `MAX_SERIES = 8` in `lib/dividendChart.ts` and two 8-entry palettes in `DividendsTab.tsx`: one number stated in two files with nothing linking them. Safe only by coincidence — raise the cap and symbols nine and ten get `palette[8] ?? OTHER_COLOR`, which is grey, indistinguishable from the *Other* bucket, three identical swatches in the legend, and nothing failing. Moved together into `lib/dividendColors.ts` beside `benchmarkColors.ts` and `sectorColors.ts`, which already had this shape. The test is deliberately the family question — build more symbols than slots and assert every colour is defined, all distinct, and the Other grey used exactly once — rather than the instance one, `SERIES_LIGHT.length === MAX_SERIES`, which survives neither direction of a change | **Superseded 2026-09-20 and worth reading as a second instance of the same lens:** the slot count and the hexes were together, but the *assignment* was still positional (`palette[stackSymbols.indexOf(sym)]` over the selected range's top eight), which is the rule `sectorColors.ts` and `benchmarkColors.ts` had each already been repaired for. The hexes now live in `index.css` as `--viz-series-*` beside the other chart palettes, `lib/dividendColors.ts` owns only the assignment, and the counts are asserted against the tokens by a test that reads `index.css` back |
