...
export interface DividendBreakdownResponse {
  ...
  /** The earliest rolling window over the WHOLE history, "YYYY-MM". Unwindowed:
   *  a year-filtered response holds only that year's windows, so nothing else in
   *  it says whether an earlier one exists — which is what decides whether a
