 ...
 *    split into money paid in, price, FX, dividends, fees/interest and an explicit remainder,
 *    as a waterfall for the range and a stacked bar per calendar year. The legs sum EXACTLY
 *    to end − start; the remainder is a named bar, never folded into another leg.
 ...
 *
 * Conventions, each a bug elsewhere first: an unknown renders `—` (ABSENT), never 0; every
 * `warnings[]` line is on the surface above the figures it qualifies, not in a tooltip; an
...
const LEG_COLORS: Record<LegKey, string> = {
  ...
  fees_interest_eur: '#dc2626',
  unsplit_eur: '#a855f7',
  unexplained_eur: '#6b7280',
}
...
function Warnings({ items, id }: { items: string[] | undefined; id: string }) {
  if (!items || items.length === 0) return null
  return (
    <ul
      role="alert"
      aria-label={`${id} caveats`}
      className="mb-3 space-y-1 rounded-md bg-yellow-50 px-3 py-2 text-xs text-yellow-800 dark:bg-yellow-950/40 dark:text-yellow-300"
    >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {w && w.start_date !== dateRange.start ? ` · valued from ${w.start_date}` : ''}
          </p>
        </div>
        <div
      ...
      <section aria-labelledby="decomposition-title">
        <Warnings items={w?.warnings} id="decomposition" />
        {decomposition.isError ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-6">
            <KpiCard
              ...
              tone={w.gain_eur === null ? 'muted' : w.gain_eur >= 0 ? 'positive' : 'negative'}
              sub={w.gain_pct === null
            ...
            <KpiCard
              ...
              tone={w.price_effect_eur === null ? 'muted' : w.price_effect_eur >= 0 ? 'positive' : 'negative'}
              sub="Earned in each holding's own currency"
            />
            <KpiCard
              label="FX"
              value={w.fx_effect_eur === null ? null : formatCurrency(w.fx_effect_eur)}
              tone={w.fx_effect_eur === null ? 'muted' : w.fx_effect_eur >= 0 ? 'positive' : 'negative'}
            ...
            <KpiCard
              label="Fees & interest"
              value={w.fees_interest_eur === null ? null : formatCurrency(w.fees_interest_eur)}
              tone={w.fees_interest_eur === null ? 'muted' : w.fees_interest_eur >= 0 ? 'positive' : 'negative'}
              sub={decomposition.data!.cash_source === 'ibkr' || decomposition.data!.cash_source === 'mixed'
                ? "From IBKR's own balance"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Left: the selected range as a waterfall from start value to end value. Right: the
            same legs per calendar year. Money paid in is a flow, not a return, and is drawn
            separately.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {decomposition.isError ? (
            ...
            <div className="grid gap-6 lg:grid-cols-2 [&>*]:min-w-0">
              <Waterfall window={w} formatCurrency={formatCurrency} compact={isCompact} />
              <YearlyLegs years={decomposition.data?.years} formatCurrency={formatCurrency} compact={isCompact} />
            </div>
          )}
            ...
            <div className={CHART_CLASS}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={rolling} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d: string) => d.slice(0, 7)} minTickGap={32} />
                  <YAxis yAxisId="ratio" tick={{ fontSize: 11 }} width={36} />
                  <YAxis yAxisId="vol" orientation="right" tick={{ fontSize: 11 }} width={40} tickFormatter={(v: number) => `${v.toFixed(0)}%`} />
                  <Tooltip
                  ...
                  />
                  <Legend />
                  <ReferenceLine yAxisId="ratio" y={0} stroke="hsl(var(--muted-foreground))" />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Every fall deeper than {DRAWDOWN_THRESHOLD_PCT}% from a running peak, on a flow-adjusted
            index — a deposit is not a recovery and a withdrawal is not a fall.
...
function Waterfall({
  window: w, formatCurrency, compact,
}: { window: DecompositionWindow | undefined; formatCurrency: (v: number) => string; compact: boolean }) {
      ...
      <div className={CHART_CLASS}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={rows} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
            <XAxis dataKey="name" tick={{ fontSize: compact ? 10 : 11 }} interval={0} angle={compact ? -30 : 0} textAnchor={compact ? 'end' : 'middle'} height={compact ? 48 : 30} />
            <YAxis tick={{ fontSize: 11 }} width={56} tickFormatter={(v: number) => formatCurrency(v).replace(/\.\d+$/, '')} />
            <Tooltip
              formatter={(_v, _n, item) => {
                const row = item?.payload as WaterfallRow | undefined
                return row ? [formatCurrency(row.value), row.name] : [ABSENT, '']
              }}
            />
            ...
            <Bar dataKey="size" stackId="w" radius={[3, 3, 0, 0]}>
              {rows.map((r) => (
                <Cell
                  key={r.name}
                  fill={r.kind === 'total' ? '#64748b' : r.kind === 'flow' ? '#94a3b8' : r.kind === 'gain' ? GAIN : LOSS}
                />
              ))}
            </Bar>
          </BarChart>
...
function YearlyLegs({
  years, formatCurrency, compact,
}: { years: DecompositionYear[] | undefined; formatCurrency: (v: number) => string; compact: boolean }) {
      ...
      <div className={CHART_CLASS}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={rows} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} stackOffset="sign">
            ...
            <YAxis tick={{ fontSize: 11 }} width={56} tickFormatter={(v: number) => formatCurrency(v).replace(/\.\d+$/, '')} />
            <Tooltip formatter={(v, n) => [v === null ? ABSENT : formatCurrency(Number(v)), String(n)]} />
            <Legend wrapperStyle={{ fontSize: compact ? 11 : 12 }} />
            <ReferenceLine y={0} stroke="hsl(var(--muted-foreground))" />
            {LEGS.map((leg) => (
              <Bar key={leg.key} dataKey={leg.key} name={leg.label} stackId="y" fill={LEG_COLORS[leg.key]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function YearTable({ years, formatCurrency }: { years: DecompositionYear[]; formatCurrency: (v: number) => string }) {
  ...
  const columns: Column<DecompositionYear>[] = [
    ...
    { key: 'fees', header: 'Fees & interest', shortHeader: 'Fees', align: 'right', tone: (y) => signed(y.fees_interest_eur), cell: (y) => money(y.fees_interest_eur) },
...
function SegmentsCard({
  data, isLoading, isError, formatCurrency, compact,
}: {
  data: ReturnType<typeof api.getSegmentAttribution> extends Promise<infer T> ? T | undefined : never
  isLoading: boolean
  isError: boolean
  ...
  const chartHeight = Math.min(compact ? 480 : 640, Math.max(200, rows.length * (compact ? 26 : 32)))
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Warnings items={data?.warnings} id="segments" />
        {isError ? (
          ...
          <>
            <p className="mb-3 text-sm">
              ...
              {data.unpriced_holdings > 0 ? ` · ${data.unpriced_holdings} unpriced, left out` : ''}
            </p>
            <div style={{ height: chartHeight }} className="w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={rows} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 4 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 11 }} tickFormatter={(v: number) => formatCurrency(v).replace(/\.\d+$/, '')} />
                  <YAxis type="category" dataKey="name" width={compact ? 110 : 170} tick={{ fontSize: compact ? 10 : 11 }} interval={0} />
                  <Tooltip formatter={(v) => [v === null ? ABSENT : formatCurrency(Number(v)), 'Gain']} />
...
function ClosedPositionsCard({
  ...
  data: ReturnType<typeof api.getClosedPositions> extends Promise<infer T> ? T | undefined : never
  isLoading: boolean
  isError: boolean
  formatCurrency: (v: number) => string
  compact: boolean
  ...
  const columns: Column<ClosedPosition>[] = [
    ...
    { key: 'after', header: 'Since sale', shortHeader: 'Since sale', align: 'right', tone: (p) => signed(p.post_sale_pct), cell: (p) => p.post_sale_pct === null ? ABSENT : `${pct(p.post_sale_pct)} · ${p.post_sale_days}d`, hint: { description: 'The quote’s move from the last close before the sale to the newest close on record. Blank when no price after the sale exists — the market-data sync prices open positions only.' } },
  ]
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Warnings items={data?.warnings} id="closed positions" />
        {isError ? (
          ...
          <>
            ...
            {chartRows.length > 0 && (
              <div style={{ height: chartHeight }} className="w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartRows} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 4 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 11 }} tickFormatter={(v: number) => formatCurrency(v).replace(/\.\d+$/, '')} />
                    <YAxis type="category" dataKey="symbol" width={compact ? 70 : 90} tick={{ fontSize: 11 }} interval={0} />
                    <Tooltip formatter={(v) => [v === null ? ABSENT : formatCurrency(Number(v)), 'Realized']} />
                    <ReferenceLine x={0} stroke="hsl(var(--muted-foreground))" />
